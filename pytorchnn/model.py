...
class LSTMTransformerModel2(nn.Module):
    ...
    def __init__(self, ntoken, ninp, nhead, nhid, nlayers, ninp_l, nhid_l, dropout=0.5, tie_weights=False):
        ...
        for _ in range(nlayers):
            self.rnn.append(getattr(nn, 'LSTM')(ninp_l, nhid_l, 1, dropout=dropout))
            self.transformerlayers.append(StandardTransformerEncoderLayer(ninp, nhead, nhid, dropout))
            pass
    ...
    def init_hidden(self, bsz):
        ...
        for _ in range(self.nlayers):
            hiddens.append((weight.new_zeros(1, bsz, self.nhid_l),
                weight.new_zeros(1, bsz, self.nhid_l)))
...
class TransformerXLDecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward, dropoutf, dropouta, **kwargs):
        ...
        self.self_attn = RelMultiHeadAttn(d_model, nhead, dropouta, **kwargs)
        self.linear1 = nn.Linear(d_model, dim_feedforward)
    ...
    def forward(self, src, src_mask=None, mems=None):
        # src.size(): (seq_length, batch_size, dim_model)  e.g (100, 32, 512)
        # print("src.size(): ", src.size())
        src2 = self.self_attn(src, src, src, attn_mask=src_mask, mems=mems)
        src = src + self.dropout1(src2)
        src = self.norm1(src)
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
        src = src + self.dropout2(src2)
...
class TransformerXLModel(nn.Module):
    ...
    def __init__(self, ntoken, ninp, nhead, nhid, nlayers, dropoute, dropouti, dropouta, dropoutf, dropouth, dropouto,
                tie_weights=True, tgt_len=None, ext_len=0, mem_len=0, clamp_len=-1):
    ...
    def forward(self, src, has_mask=True, *mems, return_h=False):
        # src.size(): (seq_length, batch_size)  e.g (100, 32)
        # print("src.size(): ", src.size())
        ...
        for i, layer in enumerate(self.transformerlayers):
            ...
            core_out = layer(core_out, pos_emb, dec_attn_mask=self.src_mask, mems=mems_i)
