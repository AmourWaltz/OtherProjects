...
decode_dir_suffix=pytorch-${model_type}-emb${embedding_dim}_hid${hidden_dim}_nly${nlayers}-drop${dropout}-${model_var}-len${seq_len}-pre${pretrain}-${level}_level-${suffix}-ng_wt${ng_weight}-itlm_wt${lm_itpr}-cross_utt${cross_utt}-${nbest}best
