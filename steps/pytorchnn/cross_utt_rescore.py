...
def get_input_and_target(sent, vocab, id, seq_len=128, pre_hyp=' ', pre_len=0):
    ...
    # if os.path.exists(os.path.join(args.outfile, 'context_best.txt')):
    #     with open(os.path.join(args.outfile, 'context_best.txt'), "r", encoding='utf-8') as f:
    #         context = f.readline()
    #         context = context.split()[:-pre_len]
    ...
    #############################
    # if os.path.exists(os.path.join(args.outfile, 'context_' + str(id) + 'best.txt')):
    #     with open(os.path.join(args.outfile, 'context_' + str(id) + 'best.txt'), "r", encoding='utf-8') as f:
    #         context = f.readline()
    #         context = context.split()[-seq_len:]
    #         context_id = list(map(int, context))
    #         input_ids = context_id + input_ids
    #         pass
    #     pass
...
def compute_sentence_score(nnlm_1, criterion, ntokens, inputs, target, model_type='LSTM', seq_len=128, 
    ...
    #############################
    # if len(inputs) > seq_len and length < seq_len:
...
def compute_scores(nbest, uttid, nnlm_1, criterion, ntokens, vocab, diction, model_type, seq_len, 
                nnlm_2=None, weight=0.0, model_var='none'):
    ...
    # open(os.path.join(args.outfile, 'context_best.txt'), "w")

    # for id in range(20):
    #     open(os.path.join(args.outfile, 'context_' + str(id) + 'best.txt'), "w")
    #     pass
    ...
    for key in uttid:
        ...
        for idx, hyp in enumerate(nbest[key]):
            print("hyp:", hyp)
            best_score = float('inf')
            for idy, pre_hyp in enumerate(pre_nbest):
                inputs, target, num_oov = get_input_and_target(hyp, vocab, idy, seq_len, pre_hyp, pre_len)
                total_oov += num_oov
                # print("inputs:", inputs)
            ...
            context = ''
            best_cont = best_cont + [vocab['<s>']]
            best_cont_list.append(best_cont)
            score_list.append(best_score)

            #############################
            # Print out selected context
            #############################
            # print("best_cont:", best_cont)
            # for word_id in best_cont:
            #     context = context + ' ' + diction[word_id]
            #     pass
            # print("context:", context + ' ' + hyp)

            # cached_hids_1.append(best_hid_1)
            # cached_hids_2.append(best_hid_2)

            if key in nbest_and_scores:
                nbest_and_scores[key].append((hyp, best_score))
                pass
            else:
                nbest_and_scores[key] = [(hyp, best_score)]
                pass
            pass

        # min_id = score_list.index(min(score_list))
        # ext_hist, _ = nbest_and_scores[key][min_id]
        # pre_len = len(ext_hist.split())

        # with open(os.path.join(args.outfile, 'context_best.txt'), "a", encoding='utf-8') as f:
        #     f.write('<s> ' + ext_hist + ' ')
        #     pass

        # for id in range(idx + 1):
        #     with open(os.path.join(args.outfile, 'context_' + str(id) + 'best.txt'), "w", encoding='utf-8') as f:
        #         f.write(' '.join(map(str, best_cont_list[id])))
        #         pass
        #     pass
...
vocab, diction = read_vocab(args.vocabulary)
ntokens = len(vocab)
...
#############################
nbest_and_scores = compute_scores(nbest, uttid, nnlm_1, criterion, ntokens, vocab, diction,
                    args.model, args.seq_len, nnlm_2, args.nnlm_weight, args.model_var)
